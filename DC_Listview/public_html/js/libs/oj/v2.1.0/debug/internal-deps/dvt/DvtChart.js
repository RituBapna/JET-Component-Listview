...
DvtChartDefaults.SKIN_ALTA = {
  ...
  'title': {'style': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 13px; color: #252525;")},
  'subtitle': {'style': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #252525;")},
  'footnote': {'style': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 11px;")},
  '_statusMessageStyle': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 13px; color: #252525;"),

  'styleDefaults': {
    'seriesEffect': 'color',
    'colors': dvt.CSSStyle.COLORS_ALTA,
    'pieCenterLabel': {'style': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;")},
    'dataLabelStyle': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;"),
    'stackLabelStyle': new dvt.CSSStyle("font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-weight: bold;"),
...
DvtChartDefaults.VERSION_1 = {
  ...
  'series': [],
  'groups': [],
  'title': {'style': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 12px; color: #003d5b; font-weight: bold'), 'halign': 'start'},
  'subtitle': {'style': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 12px; color: #003d5b;')},
  'footnote': {'style': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 10px; color: #333333;'), 'halign': 'start'},
  'titleSeparator': { 'upperColor': '#74779A', 'lowerColor': '#FFFFFF', 'rendered': 'off'},
  'touchResponse': 'auto',
  '_statusMessageStyle': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 12px; color: #003d5b; font-weight: bold'),
  ...
  'styleDefaults': {
    ...
    'pieCenterLabel': {'style': new dvt.CSSStyle('font-family: tahoma, sans-serif;')},
    ...
    'dataItemGaps': '0%',
    'dataLabelStyle': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 11px;'),
    ...
    'tooltipLabelStyle': new dvt.CSSStyle('color: #737373; padding: 0px 2px'),
    'tooltipValueStyle': new dvt.CSSStyle('color: #333333; padding: 0px 2px'),
    'stackLabelStyle': new dvt.CSSStyle('font-family: tahoma, sans-serif; font-size: 11px;')
  },
...
DvtChartAxisUtils.getGroupWidthRatios = function(chart) {
  ...
  var hasVaryingWidth = options['_hasVaryingBarWidth'];
...
DvtChartDataUtils.processDataObject = function(chart) {
  ...
  options['_hasVaryingBarWidth'] = false;
  ...
  for (var i = 0; i < seriesCount; i++) {
    ...
    if (series != null && dvt.ArrayUtils.getIndex(arSeriesStyle, series) < 0)
        ...
        if (items[j]) {
          ...
          if (!isNaN(item) && item) {
            ...
            if (item['z']) {
              ...
              if (isBLAC && item['z'] != 1)
                options['_hasVaryingBarWidth'] = true;
            }
...
DvtChartDataUtils.getSeriesStyleIndex = function(chart, series) {
  if (series) {
    var arSeriesStyle = chart.getSeriesStyleArray();
    return dvt.ArrayUtils.getIndex(arSeriesStyle, series);
  }
  else
 ...
 * @param {number} seriesIndex
 * @param {number} groupIndex
 * @param {dvt.Rectangle} availSpace The viewport space.
 * @return {boolean}
 */
DvtChartDataUtils.isMarkerInViewport = function(chart, seriesIndex, groupIndex, availSpace) {
  ...
  if (availSpace && pos && markerSize)
    return availSpace.intersects(new dvt.Rectangle(pos.x - markerSize / 2, pos.y - markerSize / 2, markerSize, markerSize));
  else
    return true;
};
...
DvtChartDataUtils.getStackCategories = function(chart, type, bIncludeHiddenSeries) {
  ...
  for (var s = 0; s < DvtChartDataUtils.getSeriesCount(chart); s++) {
    ...
    if (DvtChartDataUtils.isAssignedToY2(chart, s)) {
      if (dvt.ArrayUtils.getIndex(y2Categories, category) == -1)
        y2Categories.push(category);
    }
    else if (dvt.ArrayUtils.getIndex(yCategories, category) == -1)
 ...
 */
DvtChartPieLabelUtils.createPieCenterLabel = function(pieChart) {
  ...
  if (pieCenterLabel) {
    ...
    var centerLabel = new dvt.MultilineText(pieChart.getCtx());
    var availSpace = new dvt.Rectangle(center.x, center.y, ir * Math.sqrt(2), ir * Math.sqrt(2));
    centerLabel.setCSSStyle(new dvt.CSSStyle(centerStyle));

    centerLabel.setTextString(pieCenterLabel['text']);
    if (dvt.TextUtils.fitText(centerLabel, availSpace.w, availSpace.h, pieChart)) {
      ...
      if (centerLabel.isTruncated())
        pieChart.chart.getEventManager().associate(centerLabel, new dvt.SimpleObjPeer(centerLabel.getTextString()));
...
DvtChartPlotAreaRenderer._renderScatterBubble = function(chart, container, clipGroup, bSortBySize, availSpace) {
  ...
  DvtChartPlotAreaRenderer._filterScatterBubble(chart, bSortBySize, availSpace);
...
DvtChartPlotAreaRenderer._getMarkersForSeries = function(chart, seriesIndex, availSpace, defaultStroke) {
  ...
  for (var groupIndex = 0; groupIndex < numGroups; groupIndex++) {
    ...
    if (markerInfo.markerDisplayed) {
      ...
      marker = new dvt.SimpleMarker(context, markerInfo.shape, chart.getSkin(), markerInfo.x, markerInfo.y, markerInfo.size, markerInfo.size);
...
DvtChartPlotAreaRenderer._filterScatterBubble = function(chart, bSortBySize, availSpace) {
  ...
  for (var seriesIndex = 0; seriesIndex < seriesCount; seriesIndex++) {
    ...
    for (var groupIndex = 0; groupIndex < groupCount; groupIndex++) {
      markerInfo = DvtChartPlotAreaRenderer._getMarkerInfo(chart, seriesIndex, groupIndex, availSpace);
      if (markerInfo != null)
  ...
  for (var markerIndex = markerCount - 1; markerIndex >= 0; markerIndex--) {
    ...
    var alpha = markerInfo.markerDisplayed ? dvt.ColorUtils.getAlpha(dataColor) : 0;
