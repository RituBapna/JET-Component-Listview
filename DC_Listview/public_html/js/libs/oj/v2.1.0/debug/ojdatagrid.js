 ...
 */
"use strict";
define(['ojs/ojcore', 'jquery', 'promise', 'ojs/ojcomponentcore', 'ojs/ojdatasource-common', 'ojs/ojinputnumber', 'ojs/ojmenu', 'ojs/ojdialog'], function(oj, $)
{
/**
 ...
 * @param {Object} results - an array of headers returned from the dataSource
 ...
 * @param {string|number} totalCount - the total number of rows in the table
...
DvtDataGrid.prototype.getFocusableElementsInNode = function(node, skipTabIndexCheck)
{
    ...
    if (document.evaluate)
    {
        ...
        while (elem)
        {
            if (!elem.disabled && (skipTabIndexCheck || !elem.tabIndex || elem.tabIndex > 0 || parseInt(elem.getAttribute(attr), 10) >= 0) && this.isElementVisible(elem, node))
            {
    ...
    else
    {
        ...
        {
            elem = nodes[i];
            if (elem.tagName.match(inputRegExp) && !elem.disabled && (skipTabIndexCheck || !elem.tabIndex || elem.tabIndex >= 0 || parseInt(elem.getAttribute(attr), 10) >= 0) && this.isElementVisible(elem, node))
            {
                inputElems.push(elem);
            }
        }
...
{
    var cell, header, rowKey, details, target;
    target = /** @type {Element} */ (event.target);
...
DvtDataGrid.prototype.handleModelEvent = function(event)
{
    ...
    if (operation === 'insert')
    {
        ...
        if (cellSet != null)
        {
            ...
            if (source && oj.FlattenedTreeDataGridDataSource && source instanceof oj.FlattenedTreeDataGridDataSource)
            {
 ...
 * @return {Element|null} the column element
 * @private
 */
 ...
 * @param {Element|undefined|null} element the cell/header that is acted on
 * @returns {boolean}
 */
DvtDataGrid.prototype._handleActionableModeKeyDown = function(event, element, isHeader)
{
    ...
    else if (keyCode === DvtDataGrid.TAB_KEY)
    {
        ...
        else if (focusElems.length > 0 && target == focusElems[0] && shiftKey)
        {
 ...
 * @param {string|undefined} axis - subobject to get row/column/cell
