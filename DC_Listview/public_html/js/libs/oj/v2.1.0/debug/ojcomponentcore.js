 ...
 * @type {boolean}
 */
var _hasDocumentTouchEndHandler = false;
...
$.widget('oj.' + _BASE_COMPONENT,
{
  ...
  _AfterCreate : function ()
  {
    ...
    //do we need this?
    //this._addDocumentTouchEndHandler();
  },
...
//    this.widget().off(this.eventNamespace);
  ...
  {
    if (! _hasDocumentTouchEndHandler && oj.DomUtils.isTouchSupported())
    {
      // Called for touchend/cancel on both current element and document.  
      // Listening only on the current element isn't completely reliable, apparently due to user-friendly 
      // "close enough" heuristics on iOS and Android where the touchend can happen slightly off of the current element 
      // and the platform still counts it as a current element click.  Listening only on the document runs the risk 
      // that we won't hear it because someone eats it.  Could use capture listener to dodge that risk, but just 
      // listening on both seems to work great.
      // Tried setting a flag in current element.touchstart, so that the end listener could bail out if flag not set, 
      // but not needed, added complexity, and may have contributed to the above unreliability due to the above 
      // heuristics (platform might tolerate touchstart not being quite on the current element, similar to touchend).
      //attach both touchend and touchcancel events to document to reset _ojLastTouch

      $(document).on("touchend" + this.eventNamespace + " " + 
                     "touchcancel" + this.eventNamespace, 
                     this._hoverAndActiveEndHandler.bind(this, true));

      _hasDocumentTouchEndHandler = true;
    }
  },
  ...
  _isRealMouseEvent: function(event)
  {
    ...
    if (oj.DomUtils.isTouchSupported())
      return (Date.now() - _ojLastTouch) > 500;

    return true;
  },

  /**
   * Add touch and mouse listners to toggle oj-hover and oj-active classes
   *
   * @memberof oj.baseComponent
   * @instance
   * @protected
   * @param {!jQuery} element The element to receive the <code class="prettyprint">oj-hover</code> class on hover and the <code class="prettyprint">oj-active</code> class on active
   * @see #_RemoveHoverAndActiveListeners
   */
  _AddHoverAndActiveListeners: function(element)
  {
    var self = this;
    // see comments in the _addDocumentTouchEndHandler method
    if (oj.DomUtils.isTouchSupported())
    {
      element.on("touchstart" + this.eventNamespace,
                 this._hoverAndActiveStartHandler.bind(this, "oj-active"))
    ...
    element
      .on("mouseenter" + this.eventNamespace, 
        this._hoverAndActiveStartHandler.bind(this, "oj-hover"))
      .on("mouseleave" + this.eventNamespace, 
          this._hoverAndActiveEndHandler.bind(this, false));
    ...
    element
      ...
      .on("mouseup" + this.eventNamespace, function(event) {
        ...
        $curTarget.removeClass("oj-active");
      });
  },

  /**
   * Remove touch and mouse listners that were registered in _AddHoverAndActiveListeners
   *
   * @memberof oj.baseComponent
   * @instance
   * @protected
   * @param {!jQuery} element The same element passed to _AddHoverAndActiveListeners previously
   * @see #_AddHoverAndActiveListeners
   */
  _RemoveHoverAndActiveListeners: function(element)
  {
    if (element)
   ...
   * @private
   */
  _hoverAndActiveStartHandler: function (markerClass, event)
  {
   ...
   * @private
   */
  _hoverAndActiveEndHandler: function (isTouchEvent, event)
  {
    // Mouse handler that apply hover styles will bail out if the last touch was within the last n ms.  
    // This avoids having the mouse compatibility events set those styles 300ms after the tap, which would result in 
    // the hover style getting left on the current element, and possibly in a flash of the active style.
    if (isTouchEvent)
      _ojLastTouch = Date.now();
  ...
  {
    this._RemoveHoverAndActiveListeners(element);
    if (element) {
